...
`js/cards.js` lists every card. The order of the `CARDS` array also defines each card's position in share links, so **only ever append** new cards. The link stores the card count in one byte, so the list can hold at most 255 cards before `js/sync.js` needs a new link `VERSION`. There are 246 cards now.
